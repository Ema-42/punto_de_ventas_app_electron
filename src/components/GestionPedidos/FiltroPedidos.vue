<template>
  <div class="bg-white p-4 rounded-lg shadow-md mb-4">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="relative">
        <label class="block text-sm font-medium text-gray-700 mb-1">Buscar por ID</label>
        <input
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :class="filtrosLocales.periodo === periodo.valor 
      ...
      <!-- Opciones de exportación (más compactas) -->
      <div>
        <div class="flex justify-between items-center">
          <h3 class="text-sm font-medium text-gray-700">Opciones de exportación</h3>
          <button
        ...
        <div class="flex items-center gap-4 mt-1">
          <div class="flex items-center">
            ...
            />
            <label for="incluirDetalles" class="text-xs text-gray-700">Incluir detalles</label>
          </div>
          <div class="flex items-center">
            <input
              type="checkbox"
              ...
              class="mr-1 h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded"
            />
            <label for="incluirRanking" class="text-xs text-gray-700">Incluir ranking</label>
          </div>
          <button
            @click="$emit('exportar-pdf')"
            class="bg-red-600 text-white px-3 py-1 rounded-lg shadow-md hover:bg-red-700 transition flex items-center gap-1 text-xs ml-auto"
            :disabled="pedidosSeleccionados.length === 0"
            :class="{ 'opacity-50 cursor-not-allowed': pedidosSeleccionados.length === 0 }"
          >
        ...
        <div class="text-xs text-gray-600 mt-1">
          {{ pedidosSeleccionados.length }} pedidos seleccionados - Total: ${{ totalSeleccionados.toFixed(2) }}
        </div>
      </div>
    </div>

    <!-- Filtros avanzados (fechas y horas) -->
    <div v-if="filtrosLocales.periodo === 'personalizado'" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t">
      <!-- Rango de fechas -->
      ...
      <div class="flex gap-2">
        <div class="flex-1">
          <label class="block text-sm font-medium text-gray-700 mb-1">Hora desde</label>
          <input
            v-model="filtrosLocales.horaDesde"
            type="time"
            class="border p-2 rounded-lg shadow-sm w-full focus:ring-2 focus:ring-red-400 outline-none bg-white"
            @change="emitirCambios"
          />
        ...
        <div class="flex-1">
          <label class="block text-sm font-medium text-gray-700 mb-1">Hora hasta</label>
          <input
            v-model="filtrosLocales.horaHasta"
            type="time"
            class="border p-2 rounded-lg shadow-sm w-full focus:ring-2 focus:ring-red-400 outline-none bg-white"
            @change="emitirCambios"
          />
...
const seleccionarPeriodo = (periodo: 'dia' | 'semana' | 'todo' | 'personalizado') => {
