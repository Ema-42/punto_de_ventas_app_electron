    ...
    <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div class="bg-gray-50 p-3 rounded-lg">
          ...
          <div class="font-medium">{{ formatearFecha(pedido.fecha_creacion) }}</div>
        </div>
        <div class="bg-gray-50 p-3 rounded-lg">
        ...
        <div class="bg-gray-50 p-3 rounded-lg">
          ...
          <div class="font-medium flex items-center">
            <div class="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center mr-2">
              <span class="text-sm font-medium text-red-600">{{ pedido.mesera.nombre.charAt(0) }}</span>
            </div>
        ...
        <div class="bg-gray-50 p-3 rounded-lg">
          ...
          <div class="font-medium flex items-center">
            <div class="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center mr-2">
              <span class="text-sm font-medium text-green-600">{{ pedido.cajero.nombre.charAt(0) }}</span>
            </div>
      ...
      <div class="mb-4">
        ...
        <div class="overflow-x-auto bg-white border rounded-lg">
          <table class="w-full border-collapse">
            ...
            <tbody>
              <tr v-for="detalle in pedido.detalles" :key="detalle.id" class="border-b">
                <td class="p-3">
            ...
            <tfoot>
              <tr class="bg-gray-50">
                ...
                <td class="p-3 text-right font-bold">${{ parseFloat(pedido.total).toFixed(2) }}</td>
              </tr>
            </tfoot>
          </table>
        </div>
...
const getTipoPagoClase = (tipoPago?: string): string => {
  const clases: { [key: string]: string } = {
    'EFECTIVO': 'bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-medium',
    'TARJETA': 'bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs font-medium',
    'TRANSFERENCIA': 'bg-purple-100 text-purple-800 px-2 py-1 rounded-full text-xs font-medium',
    'MIXTO': 'bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs font-medium'
  };
  
  return tipoPago ? (clases[tipoPago] || 'bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-xs font-medium') : 'text-gray-500';
};
